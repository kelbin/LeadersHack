...
Используется UIKit. Верстка построенна на собственном решение RPCEngine, который верстает интерфейсы вертикальными ячейками, по входящим моделькам. Преврашением моделек в ячейки -- ответвенность DataSource. Ячейки верстаются с помощью Snapkit
...
На этапе инициализации модуля, происходит связвания на данные из интерактора. Переменная `geoZones` определяет, как будет отображен интерфейс во вью контролере. Сам вью контроллер подписывается на эту переменную следующим образом
...
Роль интерактора иметь входные параметры и следить за актуальностью данных в модулях. обновляя их через бекенд сервисы. Входные параметры интерактора выглядит следующим образом
